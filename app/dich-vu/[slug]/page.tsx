...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> { const { slug } = await params; const s = serviceByViSlug[slug] as SeoService | undefined; if (!s) return {}; const path = servicePath(s, "vi"); const image = `${siteUrl}${s.image}`; return { title: s.vi.title, description: s.vi.description, alternates: { canonical: path, languages: { "vi-VN": path, en: servicePath(s, "en"), "x-default": path } }, openGraph: { type: "website", locale: "vi_VN", url: `${siteUrl}${path}`, title: s.vi.title, description: s.vi.description, images: [{ url: image, alt: s.vi.name }] }, twitter: { card: "summary_large_image", title: s.vi.title, description: s.vi.description, images: [image] } }; }
