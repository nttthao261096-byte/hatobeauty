...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> { const { slug } = await params; const s = serviceByEnSlug[slug] as SeoService | undefined; if (!s) return {}; const path = servicePath(s, "en"); const image = s.image; return { title: s.en.title, description: s.en.description, alternates: { canonical: path, languages: { "vi-VN": servicePath(s, "vi"), en: path, "x-default": servicePath(s, "vi") } }, openGraph: { type: "website", locale: "en_US", url: `${siteUrl}${path}`, title: s.en.title, description: s.en.description, images: [{ url: image, alt: s.en.name }] }, twitter: { card: "summary_large_image", title: s.en.title, description: s.en.description, images: [image] } }; }
