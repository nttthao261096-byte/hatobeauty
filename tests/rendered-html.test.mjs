...
test("server-renders the redesigned Hato Beauty experience", async () => {
  ...
  assert.match(html, /Nhận ưu đãi mới mỗi tháng/i);
...
test("ships the new brand hierarchy and accessible booking form", async () => {
  ...
  assert.doesNotMatch(html, /class="announcement"/i);
  assert.doesNotMatch(html, /class="hero-playback"/i);
  assert.doesNotMatch(html, /không thay thế thăm khám da liễu/i);
  assert.match(source, /Thiết bị hiện đại/i);
  assert.match(source, /Đội ngũ chuyên nghiệp/i);
  assert.match(source, /Dịch vụ cá nhân hóa/i);
  assert.match(source, /Không gian thư giãn/i);
  assert.match(styles, /\.seo-book-link span\s*\{\s*display:\s*inline\s*!important/i);
  assert.match(html, /Tỏa sáng là chính bạn/i);
  assert.match(html, /class="footer-intro footer-newsletter"/i);
  assert.doesNotMatch(html, /Hẹn gặp bạn tại Hato Beauty/i);
  ...
  const heroSource = await readFile(new URL("../app/HeroMedia.tsx", import.meta.url), "utf8");
  assert.match(heroSource, /className=\{`hero-video hero-video-/i);
  assert.match(heroSource, /\/video\/hero-care-beige-clinic\.mp4/i);
  assert.doesNotMatch(source, /hero-care-image/i);
  assert.doesNotMatch(styles, /hero-signature/i);
  assert.ok(heroSource.indexOf('/video/hero-head-spa.mp4') < heroSource.indexOf('/video/hero-care-beige-clinic.mp4'));
  assert.match(heroSource, /\/video\/hero-hair-removal\.mp4/i);
  assert.match(heroSource, /\/video\/hero-head-spa\.mp4/i);
  assert.match(heroSource, /\/video\/hero-brow-warm\.mp4/i);
