...
test("ships the new brand hierarchy and accessible booking form", async () => {
  ...
  assert.match(source, /Công nghệ phù hợp/i);
  assert.match(source, /Không gian dễ chịu/i);
  assert.match(source, /Thông tin minh bạch/i);
  assert.match(source, /Lắng nghe cẩn trọng/i);
  assert.match(styles, /\.seo-book-link span\s*\{\s*display:\s*inline\s*!important/i);
  assert.match(html, /Tỏa sáng là chính bạn/i);
  assert.match(html, /class="footer-intro"/i);
  assert.match(html, /Hẹn gặp bạn tại Hato Beauty/i);
